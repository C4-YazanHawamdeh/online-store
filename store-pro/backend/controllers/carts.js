...
const getMyCart = (req, res) => {
  const query = `SELECT  carts.*,products.name,products.image,products.description,products.price FROM carts inner Join products on product_id = products.id  WHERE carts.is_deleted=0 AND carts.user_id=${req.token.userId} ;`;
